...
export default function TakeInternalExamPage() {
  ...
  const handleSubmitExam = async (isAutoSubmit: boolean = false) => {
    ...
    try {
      const res = await fetch('/api/cbt/internal', {
        method: 'POST',
  ...
  if (loading) {
    return <div className="fixed inset-0 bg-slate-50 flex items-center justify-center"><Loader2 className="w-12 h-12 animate-spin text-[#0033A0]" /></div>;
  }
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 max-w-md w-full text-center animation-fade-in">
          ...
          <div className="bg-slate-50 rounded-xl p-6 border border-slate-100 mb-8">
            ...
            <div className={`mt-4 inline-block px-4 py-1.5 rounded-full text-sm font-bold ${
              percentage >= 75 ? 'bg-emerald-100 text-emerald-800' :
              percentage >= 50 ? 'bg-[#0033A0]/20 text-[#0033A0]' :
          ...
          <Link href="/cbt" className="block w-full bg-[#0A192F] hover:bg-[#060F1D] text-white font-bold py-3 rounded-xl transition-colors">
            Return to Portal
          </Link>
        </div>
      </div>
    ...
    <div className="bg-slate-50 min-h-screen pb-32">
      <div className="sticky top-0 bg-white border-b border-slate-200 shadow-sm z-50 px-4 py-4 md:px-8">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="font-black text-slate-900 text-lg md:text-xl">Internal Examination</h1>
            <p className="text-xs md:text-sm font-bold text-[#0033A0]">{studentId}</p>
          </div>
          ...
          <>
                        ...
                        return (
                          <button
                            ...
                            className={`text-left p-4 rounded-xl border-2 transition-all flex items-start gap-3 ${
                              isSelected 
                                ? 'bg-[#0033A0]/10 border-[#0033A0] text-[#0033A0]' 
                                : 'bg-white border-slate-200 text-slate-600 hover:border-[#0033A0]/50 hover:bg-slate-50'
                            }`}
                          >
                            <div className={`w-5 h-5 shrink-0 rounded-full border-2 flex items-center justify-center mt-0.5 ${
                              isSelected ? 'border-[#0033A0]' : 'border-slate-300'
                            }`}>
            ...
            <div className="pt-8 border-t-2 border-slate-200 flex justify-end">
              <button 
                ...
                className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white font-black text-lg px-12 py-5 rounded-2xl transition-all shadow-lg hover:shadow-xl flex items-center gap-3 disabled:cursor-not-allowed"
              >
